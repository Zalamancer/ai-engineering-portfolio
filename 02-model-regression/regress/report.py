...
<tr><th>backend / judge</th><td>{{ c.backend or 'openai' }} · {% if c.judge_model %}{{ c.judge_model }} <small>(summary scores are LLM judgments)</small>{% else %}<small>no judge — jev returns a typed choice, the summary dimension is not applicable</small>{% endif %}</td><th>timestamp</th><td>{{ c.created }}</td></tr>
...
<table><tr><th>case</th><th>expected</th><th>predicted</th><th>{{ 'confidence' if c.backend == 'jev' else 'summary score' }}</th><th>output</th><th>problem</th></tr>
{% for f in failures %}<tr><td>{{ f.case_id }}<br><small>{{ f.difficulty }}</small></td><td>{{ f.expected_category }}</td><td>{{ f.predicted_category or 'INVALID' }}</td><td>{{ ('%.2f' % f.confidence) if f.confidence is not none else f.summary_score }}</td><td><pre>{{ f.raw_output }}</pre></td><td>{{ f.problem }}</td></tr>{% endfor %}</table>
...
def render_report(cand_meta: dict, cand_cases: dict[str, dict], comparison: dict | None, drift: dict | None,
    ...
    if cand_meta.get("confidence_mean") is not None:
        scorecard += [row("mean confidence (jev)", "confidence_mean", fmt=lambda x: f"{x:.3f}"),
                      row("mean confidence on wrong answers (lower is better)", "confidence_mean_wrong", fmt=lambda x: f"{x:.3f}", lower_is_better=True)]
    failures = []
